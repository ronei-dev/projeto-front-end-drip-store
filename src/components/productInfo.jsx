...
const ProductInfo = ({ product }) => {
    ...
    const [selectedSize, setSelectedSize] = useState(product.size ? product.size[0] : null);
    ...
    return (
        <div className="flex flex-col">
                ...
                <div className="mt-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {product.sizes.map(size => (
                            <button key={size} onClick={() => setSelectedSize(size)} className={`w-11 h-11 flex items-center justify-center border rounded-md transition-all text-sm ${selectedSize === size ? 'bg-pink-600 text-white border-pink-600 font-bold' : 'bg-white text-gray-700 border-gray-300 hover:border-gray-500'}`}>
                                {size}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {product.colors.map(color => (
                            <button key={color} onClick={() => setSelectedColor(color)}
                                className={`w-8 h-8 rounded-full border-2 transition-transform duration-150 transform ${selectedColor === color ? 'border-pink-600 scale-110' : 'border-gray-400'
                                    }`}
            ...
            <button
                ...
                className={`w-72 font-bold py-2 rounded-lg mt-6 transition-colors shadow-md ${isProductInCart? 'bg-yellow-600 text-white': 'bg-yellow-500 text-black hover:bg-yellow-600'}`}>
