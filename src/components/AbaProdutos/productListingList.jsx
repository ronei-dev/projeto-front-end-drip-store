...
const ProductListingList = ({ products }) => {
    ...
    return (
        <div className="gird grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map((product) => {
                ...
                return (
                    <div key={product.id} className="bg-white border border-gray-200 rounded-lg shadow-md flex flex-col justify-between transition-shadow hover:shadow-xl">
                        ...
                        <div className=" p-2 flex flex-col grow">
                            <span className="text-ts text-gray-500">{product.category}</span>
                            <h3 className="text-base font-semibold text-gray-800 my-1 grow">{product.name}</h3>
                        ...
                        <div className="px-2 pb-2">
                            <button onClick={handleToggleCart} className={`w-full font-semibold py-1.5 text-sm rounded-lg transition mb-2 ${isInCart
                                    ? 'bg-green-600 text-white hover:bg-green-700'
                                    : 'bg-pink-600 text-white hover:bg-pink-700'
                                }`}>
                                    {isInCart ? 'Adicionado ao carrinho' : 'Adicionar ao carrinho'}
                            </button>
                            <Link to={`/produto/${product.id}`} className="w-full block text-center bg-gray-200 text-gray-700 font-semibold py-1.5 text-sm rounded-lg hover:bg-gray-300 transition">
                                Ver mais
                            </Link>
                        </div>
                    </div>
